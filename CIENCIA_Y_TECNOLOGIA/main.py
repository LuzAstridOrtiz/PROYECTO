...
nltk.data.path.append(r'C:\Users\Astrid Ortiz\AppData\Roaming\nltk_data')
...
def load_movies():
    ...
    df = pd.read_csv("Dataset/herramientas_ia_espanol.csv")[['id', 'title', 'year', 'category', 'rating', 'overview']]
...
app = FastAPI(title="Mi aplicación recomendación de IA", version="1.0.0")
...
# Cuando entremos en el navegador a http://127.0.0.1:8000/ veremos un mensaje de bienvenida
    return HTMLResponse('<h1>Bienvenido a la API de recomendación de IA</h1>')
...
def get_movies():
    ...
    return movies_list or HTTPException(status_code=500, detail="No hay datos de IA disponibles")
